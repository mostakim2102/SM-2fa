...
def send_message(chat_id, text, reply_markup=None, disable_preview=False):
    ...
    # এখানে টাইপ চেক করে অটোমেটিক স্ট্রিং এ কনভার্ট করা হচ্ছে যেন টেলিগ্রাম রিজেক্ট না করে
    if reply_markup:
...
def edit_message_text(chat_id, message_id, text, reply_markup):
    payload = {
        ...
        "text": text,
        "parse_mode": "Markdown",
        "reply_markup": reply_markup if isinstance(reply_markup, dict) else json.loads(reply_markup)
    }
...
def handle_update(update):
    if "message" in update and "text" in update["message"]:
        ...
        if text == "/start":
            ...
            # getMe রিকোয়েস্টের উপর নির্ভরতা বাদ দিয়ে সেফ উপায়ে ইউজারনেম নেওয়া হচ্ছে
            bot_username = "2FA_Code_Generator_bot" # <- এখানে আপনার বটের অরিজিনাল ইউজারনেমটি (@ ছাড়া) লিখে দিন সবচেয়ে ভালো ফলের জন্য
            
            # প্রধান মেসেজ (ইউজারের নিজস্ব চ্যাটের জন্য)
            share_text = (
            ...
            # বন্ধুদের চ্যাটে প্রিভিউ সহ সুন্দরভাবে উপস্থাপনের টেক্সট
            share_msg = f"🔐 এই বটটি দিয়ে খুব সহজেই ফেসবুক বা জিমেইলের ২-ফ্যাক্টর (2FA) কোড বের করা যায়। ট্রাই করে দেখতে পারো!\n\n🔗 https://t.me/{bot_username}"
            
            # ইউআরএল কম্পোনেন্টগুলো আলাদা এনকোড করে সেফলি মার্জ করা হয়েছে
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write("Permanent Multi-Admin Bot is Active! 3".encode())
