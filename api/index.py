...
def base32_decode(base32_str):
    alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ23234567"
...
def send_message(chat_id, text, reply_markup=None, disable_preview=False):
    ...
    # টেলিগ্রামের নতুন API রুলস অনুযায়ী প্রিভিউ ফোর্স-এনাবল করার স্ট্যান্ডার্ড কোড
...
def handle_update(update):
    if "message" in update and "text" in update["message"]:
        ...
        if text == "/start":
            ...
            # বটের নিজস্ব চ্যাটে দেখানোর মেসেজ
            share_text = (
                "📢 <b>Fastest 2FA TOTP Generator Bot</b>\n\n"
                ...
                f"🔗 <b>বট লিংক:</b> https://t.me/{bot_username}\n\n"
                "নিচের বাটনে ক্লিক করে বন্ধুদের সাথে শেয়ার করুন! 👇"
            )
            
            # ইনবক্সে যেন প্রিভিউ মাস্ট আসে, তাই টেক্সটের ভেতরেই হাইপারলিংক করে দেওয়া হলো
            # এটি টেলিগ্রামকে বাধ্য করবে বটের ডেসক্রিপশন এবং প্রোফাইল পিকচারের বড় কার্ডটি জেনারেট করতে
            share_msg = (
                f"🔐 এই বটটি দিয়ে খুব সহজেই ফেসবুক বা জিমেইলের ২-ফ্যাক্টর (2FA) কোড বের করা যায়। ট্রাই করে দেখতে পারো!\n\n"
                f"👉 <b>বট লিংক এখানে:</b> https://t.me/{bot_username}"
            )
            
            encoded_msg = urllib.parse.quote_plus(share_msg)
            
            # ডিরেক্ট টেক্সট শেয়ার লিংক জেনারেশন
            full_share_link = f"https://t.me/share/url?url={encoded_msg}"
            
            inline_keyboard = {
                "inline_keyboard": [
                    [{"text": "Share now 🚀", "url": full_share_link}]
                ]
            }
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write("Permanent Multi-Admin Bot is Active! 01".encode())
