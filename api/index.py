...
import urllib.parse  # এনকোডিং এর জন্য নিশ্চিত করা হলো
import json
import time
import hashlib
import hmac
...
def handle_update(update):
    if "message" in update and "text" in update["message"]:
        ...
        if text == "/start":
            ...
            # প্রধান মেসেজ যা ইউজারের চ্যাটে দেখাবে
            share_text = (
            ...
            # শেয়ার ডায়ালগে প্রিভিউ সহ সুন্দরভাবে উপস্থাপনের জন্য টেক্সট ও লিংক জেনারেশন
            target_link = f"https://t.me/{bot_username}"
            share_msg = f"🔐 এই বটটি দিয়ে খুব সহজেই ফেসবুক বা জিমেইলের ২-ফ্যাক্টর (2FA) কোড বের করা যায়। ট্রাই করে দেখতে পারো!"
            
            # সঠিক ও নিখুঁত ইউআরএল এনকোডিং (কোয়ারি স্ট্রিং বাফারিং এরর এড়াতে quote_plus)
            encoded_text = urllib.parse.quote_plus(share_msg)
            share_url = f"https://t.me/share/url?url={urllib.parse.quote_plus(target_link)}&text={encoded_text}"
            
            inline_keyboard = {
                "inline_keyboard": [
                    [{"text": "Share now 🚀", "url": share_url}]
                ]
            }
            # এখানে সরাসরি json.dumps রিলে করা হয়েছে যা ব্যাকএন্ড ক্র্যাশ হওয়া আটকাবে
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write("Permanent Multi-Admin Bot is Active! 2".encode())
